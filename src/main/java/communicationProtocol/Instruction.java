...
public class Instruction extends Message {
  ...
  public void insertInstruction(Card card, Keyword action, Zone srcZone, Zone trgZone) {
    setCard(card);
    setAction(action);
    setSourceZone(srcZone);
    setTargetZone(trgZone);
    super.setChatMessage(card.getProperties().get("Name") + " " + action.toString() + " FROM "
        + srcZone.getZoneType().toString() + " TO " + trgZone.getZoneType().toString());
  }
   ...
   */
  public void setInstructionProperties(HashMap<String, String> instructionProperties) {
